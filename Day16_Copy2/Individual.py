...
class Individual:
    ...
    def evaluate_fitness(self):
        ...
        for i, valve_name in enumerate(self.__representation):
            ...
            num_correct_moves = num_correct_moves + 1

            minute = minute + 1
            prev_valve = valve

        self.__cached_is_invalid = is_invalid
        if self.__cached_is_invalid:
            self.__cached_fitness = num_correct_moves / (self.__settings.time_limit + 1)
        else:
            self.__cached_fitness = released_pressure - penalty_points
        return self.__cached_fitness
